...
public class MadievalDimensionTeleporter {
	...
	public static void registerPointOfInterest(RegisterEvent event) {
		event.register(Registries.POINT_OF_INTEREST_TYPE, registerHelper -> {
			PoiType poiType = new PoiType(ImmutableSet.copyOf(TheMiddleAgesModBlocks.MEDIEVAL_DIMENSION_PORTAL.get().getStateDefinition().getPossibleStates()), 0, 1);
			registerHelper.register(Identifier.parse("the_middle_ages:medieval_dimension_portal"), poiType);
			...
			for (int box = -1; box < 2; box++) {
				for (int width = 0; width < 2; width++) {
					for (int height = -1; height < 3; height++) {
						BlockState blockState = height < 0 ? Blocks.MOSSY_STONE_BRICKS.defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int width = -1; width < 3; width++) {
			for (int height = -1; height < 4; height++) {
				if (width == -1 || width == 2 || height == -1 || height == 3) {
					...
					this.level.setBlock(mutable, Blocks.MOSSY_STONE_BRICKS.defaultBlockState(), 3);
				}
			}
		}
		BlockState portalBlockState = TheMiddleAgesModBlocks.MEDIEVAL_DIMENSION_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, portalAxis);
		for (int width = 0; width < 2; width++) {
