...
public class ExecuteClicksSellerProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (!world.getEntitiesOfClass(BreadSellerEntity.class, new AABB(Vec3.ZERO, Vec3.ZERO).move(new Vec3(x, y, z)).inflate(6 / 2d), e -> true).isEmpty()) {
			...
			if (!(findEntityInWorldRange(world, BreadSellerEntity.class, x, y, z, 6)).level().isClientSide())
				(findEntityInWorldRange(world, BreadSellerEntity.class, x, y, z, 6)).discard();
