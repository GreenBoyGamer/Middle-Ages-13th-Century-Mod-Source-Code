		...
		if (!world.isClientSide()) {
			...
			if ((entity instanceof SoldierEntity _datEntI ? _datEntI.getEntityData().get(SoldierEntity.DATA_fightingState) : 0) == 0) {
				if (actionState == 0) {
					...
					if (actionState < 3) {
						AttackDamage = entity instanceof LivingEntity _livingEntity89 && _livingEntity89.getAttributes().hasAttribute(Attributes.ATTACK_DAMAGE) ? _livingEntity89.getAttribute(Attributes.ATTACK_DAMAGE).getValue() : 0;
