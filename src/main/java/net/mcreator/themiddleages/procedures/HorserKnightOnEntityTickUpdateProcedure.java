	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!world.isClientSide()) {
			...
			if (actionState == 0) {
				...
				if (actionState < 3) {
					AttackDamage = entity instanceof LivingEntity _livingEntity137 && _livingEntity137.getAttributes().hasAttribute(Attributes.ATTACK_DAMAGE) ? _livingEntity137.getAttribute(Attributes.ATTACK_DAMAGE).getValue() : 0;
					...
					if (entity.getPersistentData().getDoubleOr("actionTicks", 0) > 20) {
						...
						if (!entity.level().isClientSide())
							entity.discard();
						if (world instanceof ServerLevel _level) {
							Entity entityToSpawn = TheMiddleAgesModEntities.KNIGHT.get().spawn(_level, BlockPos.containing(x, y, z), EntitySpawnReason.MOB_SUMMONED);
			...
			if ((entity instanceof KnightRidingHorseEntity _datEntI ? _datEntI.getEntityData().get(KnightRidingHorseEntity.DATA_alive) : 0) == 1) {
				entity.getPersistentData().putDouble("SpawnTicks", (entity.getPersistentData().getDoubleOr("SpawnTicks", 0) + 1));
				if (entity.getPersistentData().getDoubleOr("SpawnTicks", 0) >= 50) {
					if (entity instanceof KnightRidingHorseEntity _datEntSetI)
