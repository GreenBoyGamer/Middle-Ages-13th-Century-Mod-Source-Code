	...
	public void setupAnim(AvatarRenderState renderState, CallbackInfo ci) {
		...
		if (!master.equals("the_middle_ages")) {
			if (!TheMiddleAgesModPlayerAnimationAPI.animations.isEmpty())
				TheMiddleAgesModPlayerAnimationAPI.animations.clear();
			return;
		}
		Player player = (Player) renderState.getRenderData(TheMiddleAgesModPlayerAnimationAPI.ClientAttachments.PLAYER);
		if (player == null)
		...
		if (data.getBooleanOr("ResetPlayerAnimation", false)) {
			...
			TheMiddleAgesModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		TheMiddleAgesModPlayerAnimationAPI.PlayerAnimation animation = TheMiddleAgesModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = TheMiddleAgesModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				TheMiddleAgesMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			TheMiddleAgesModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, TheMiddleAgesModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			TheMiddleAgesModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
