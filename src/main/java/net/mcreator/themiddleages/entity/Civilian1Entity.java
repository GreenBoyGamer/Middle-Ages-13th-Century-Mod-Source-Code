...
public class Civilian1Entity extends Monster {
	...
	public void die(DamageSource source) {
		super.die(source);
		Civilian1EntityDiesReachedProcedure.execute(this);
	}

	@Override
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata) {
		SpawnGroupData retval = super.finalizeSpawn(world, difficulty, reason, livingdata);
		Civilian1OnInitialEntitySpawnProcedure.execute(this);
		return retval;
	}
	...
	public void addAdditionalSaveData(CompoundTag compound) {
		...
		compound.putInt("DataMoney", this.entityData.get(DATA_Money));
		compound.putInt("DataVegetables", this.entityData.get(DATA_Vegetables));
		compound.putInt("Datacarry", this.entityData.get(DATA_carry));
	}
	...
	public void readAdditionalSaveData(CompoundTag compound) {
		...
		if (compound.contains("DataMoney"))
			this.entityData.set(DATA_Money, compound.getInt("DataMoney"));
		if (compound.contains("DataVegetables"))
			this.entityData.set(DATA_Vegetables, compound.getInt("DataVegetables"));
		if (compound.contains("Datacarry"))
			this.entityData.set(DATA_carry, compound.getInt("Datacarry"));
	}
